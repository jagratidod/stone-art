...
const OurCreationsManagementPage = () => {
  const [creations, setCreations] = useState([...ourCreations])
  const [showAddModal, setShowAddModal] = useState(false)
  const [showEditModal, setShowEditModal] = useState(false)
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false)
  const [selectedCreation, setSelectedCreation] = useState(null)
  const [creationToDelete, setCreationToDelete] = useState(null)
  const [sectionHeading, setSectionHeading] = useState('OUR CREATIONS')

  useEffect(() => {
    // In production, fetch from API
  }, [])

  const handleAddCreation = (newCreation) => {
    const newId = Math.max(...creations.map(c => parseInt(c.id?.replace(/\D/g, '') || '0')), 0) + 1
    const creationWithId = { 
      ...newCreation, 
      id: `${newCreation.category.toLowerCase().replace(/\s+/g, '-')}-${newId}` 
    }
    setCreations([...creations, creationWithId])
    setShowAddModal(false)
  }

  const handleEditCreation = (updatedCreation) => {
  ...
  const confirmDelete = () => {
    if (creationToDelete) {
      setCreations(creations.filter(c => c.id !== creationToDelete.id))
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-800">Our Creations Management</h1>
          <button
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {creations.map((creation) => (
                  <tr key={creation.id} className="hover:bg-gray-50">
        ...
        {/* Edit Modal */}
        {showEditModal && selectedCreation && (
          <CreationEditModal
            creation={selectedCreation}
            onSave={handleEditCreation}
            onClose={() => {
              setShowEditModal(false)
              setSelectedCreation(null)
            }}
        ...
        {/* Delete Confirmation */}
        {showDeleteConfirm && creationToDelete && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
              <h3 className="text-lg font-bold text-gray-800 mb-4">Confirm Delete</h3>
              <p className="text-gray-600 mb-6">
                Are you sure you want to delete "{creationToDelete.name}"? This action cannot be undone.
              </p>
...
// Creation Edit Modal Component
const CreationEditModal = ({ creation, onSave, onClose, isAdd }) => {
  const [formData, setFormData] = useState({
    name: creation?.name || '',
    category: creation?.category || categories[0] || '',
    image: creation?.image || '',
    imageFile: null
  })
  const [previewImage, setPreviewImage] = useState(creation?.image || null)
  ...
  const handleSubmit = (e) => {
    ...
    const creationData = {
      ...formData,
      id: creation?.id || `${formData.category.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`,
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-800">
              {isAdd ? 'Add New Creation' : 'Edit Creation'}
            </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-4">
              <button
                ...
                style={{ backgroundColor: '#8B7355' }}
              >
                {isAdd ? 'Add Creation' : 'Save Changes'}
              </button>
              <button
                type="button"
                onClick={onClose}
...
export default OurCreationsManagementPage
